...
6. **Store**: Saves to Supabase with HNSW-indexed vectors
...
- Link metadata fetching uses 5 concurrent requests
- Embedding generation uses 3 concurrent requests
- Adjust in `fetchAllLinkMetadata(concurrency, batchSize)`
